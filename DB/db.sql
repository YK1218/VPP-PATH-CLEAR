-- ======================================================================================
-- PathClear Database Schema (DB/db.sql)
...
-- --------------------------------------------------------------------------------------
-- SAFE CLEANUP OF LEGACY / PARTIAL TABLES
-- Ensures clean execution even if older/partial tables were created previously.
-- --------------------------------------------------------------------------------------
DROP TABLE IF EXISTS navigation_breadcrumbs CASCADE;
DROP TABLE IF EXISTS navigation_sessions CASCADE;
DROP TABLE IF EXISTS route_micro_segments CASCADE;
DROP TABLE IF EXISTS routes CASCADE;
DROP TABLE IF EXISTS verifications CASCADE;
DROP TABLE IF EXISTS verification_logs CASCADE;
DROP TABLE IF EXISTS hazards CASCADE;
DROP TABLE IF EXISTS entrance_dimension_pins CASCADE;
...
DROP TABLE IF EXISTS accessibility_profiles CASCADE;
...
-- Defensive Alter in case hazards was created prior without city_id
ALTER TABLE hazards ADD COLUMN IF NOT EXISTS city_id VARCHAR(50) REFERENCES cities(id) ON DELETE SET NULL;
ALTER TABLE hazards ADD COLUMN IF NOT EXISTS title VARCHAR(200);
ALTER TABLE hazards ADD COLUMN IF NOT EXISTS location_description VARCHAR(255);
